/* ============================================================================
 * ADAM Directors Dashboard v0.2 — embedded demo dataset
 * ----------------------------------------------------------------------------
 * Mirrors the schemas used by the live NetStreamX backend:
 *   agents/agent-registry.json
 *   docs/directors.json (5 mandatory + CPO + CTO)
 *   boss/boss-config.json
 *   flight_recorder/flight-recorder-schema.json
 *   intent/intent-object-schema.json
 *
 * Used as a deterministic fallback when the /api/dashboard endpoints are
 * unreachable (file://, kiosk, or pre-boot demo). The shape matches what the
 * server returns from /api/dashboard/state?mode=demo so the renderer is
 * exactly the same code path either way.
 * ========================================================================== */
window.ADAM_DEMO = (function () {
  ...
  const directors = [
    ...
    { id: "legal_director", title: "Legal Director",  name: "Michael Lamb (acting)",   domain: "Regulatory & jurisdictional compliance",     boss_dims: ["regulatory_impact", "rights_certainty"],       cap_usd: null,   emergency_override: false, active: true, edit_all: false },
    ...
    { id: "ciso",           title: "CISO",            name: "Michael Lamb (acting)",   domain: "Security posture, trust boundaries",         boss_dims: ["security_impact", "sovereignty_action"],       cap_usd: null,   emergency_override: true,  active: true, edit_all: true  },
  ...
  /* Deterministic synthetic agent state */
  let seed = 42;
  const rnd = () => { seed = (seed * 9301 + 49297) % 233280; return seed / 233280; };
  const statesFor = () => { const r = rnd(); if (r > 0.93) return "down"; if (r > 0.78) return "escalation"; return "autonomous"; };
  const agent_state = {};
  Object.values(agent_classes).forEach(cls => {
    cls.agents.forEach(a => {
      const status = statesFor();
      agent_state[a.id] = {
        id: a.id, status,
        ...
        current_step: status === "autonomous"
          ...
          : status === "escalation" ? "awaiting director approval" : "offline — investigating"
      };
    });
  });
  ...
  /* Director approval queue — 7 entries, one per active director */
  const queue = [
    { intent_id: "11111111-1111-4000-8000-000000000001", queued_at: now(), owning_director: "cfo", tier: "HIGH", score: 63,
      ...
      non_idempotent: true, triggered_by: ["financial_exposure","non_idempotent_penalty"],
      alternatives: [
    ...
    { intent_id: "22222222-2222-4000-8000-000000000002", queued_at: now(), owning_director: "ciso", tier: "OHSHAT", score: 81,
      summary: "Isolate compromised egress node us-east-staging-3 after anomalous outbound traffic.",
      ...
      non_idempotent: true, triggered_by: ["security_impact","critical_override"],
      alternatives: [
    ...
    { intent_id: "33333333-3333-4000-8000-000000000003", queued_at: now(), owning_director: "legal_director", tier: "HIGH", score: 58,
      ...
      non_idempotent: false, triggered_by: ["regulatory_impact","novel_interpretation"],
      alternatives: [
    ...
    { intent_id: "44444444-4444-4000-8000-000000000004", queued_at: now(), owning_director: "market_director", tier: "HIGH", score: 54,
      ...
      non_idempotent: true, triggered_by: ["reputational_risk","financial_exposure","non_idempotent_penalty"],
      alternatives: [
    ...
    { intent_id: "55555555-5555-4000-8000-000000000005", queued_at: now(), owning_director: "ceo", tier: "OHSHAT", score: 78,
      ...
      non_idempotent: true, triggered_by: ["doctrinal_alignment","critical_override","sovereignty_action"],
      alternatives: [
    ...
    { intent_id: "66666666-6666-4000-8000-000000000006", queued_at: now(), owning_director: "cpo", tier: "HIGH", score: 56,
      ...
      non_idempotent: false, triggered_by: ["doctrinal_alignment","financial_exposure"],
      alternatives: [
    ...
    { intent_id: "77777777-7777-4000-8000-000000000007", queued_at: now(), owning_director: "cto", tier: "ELEVATED", score: 47,
      ...
      non_idempotent: true, triggered_by: ["security_impact","non_idempotent_penalty"],
      alternatives: [
  ...
  const routing_24h = { soap: 4821, moderate: 612, elevated: 58, high: 14, ohshat: 2 };

  return {
    meta: {
      company: "NetStreamX", profile_type: "test", adam_version: "1.4",
      boss_formulas_version: "3.2", doctrine_version: "1.1.0-test", generated_at: now()
    },
    directors, agent_classes, agent_state, twin_usage, queue, flight_recorder, routing_24h,
    boss: {
      dimensions: {
      ...
      routing_thresholds: {
        soap:     { min: 0,  max: 10, label: "SOAP" },
        moderate: { min: 11, max: 30, label: "MODERATE" },
        elevated: { min: 31, max: 50, label: "ELEVATED" },
        high:     { min: 51, max: 75, label: "HIGH" },
        ohshat:   { min: 76, max: 100,label: "OHSHAT" }
      }
    }
  };
})();
