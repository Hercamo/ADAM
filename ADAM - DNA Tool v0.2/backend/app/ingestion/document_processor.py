...
class DocumentProcessor:
    ...
    async def process_file(self, filename: str, content: bytes) -> Dict[str, Any]:
        ...
        # path traversal (CWE-22).  Also reject any name that resolves outside
        # the configured upload directory after normalisation.
        ...
        # Save file under the upload directory using only the sanitised basename
        upload_root = os.path.realpath(self.upload_dir)
        filepath = os.path.realpath(os.path.join(upload_root, safe_name))
        if not filepath.startswith(upload_root + os.sep):
            raise ValueError("Resolved upload path escapes upload directory")
    ...
    def _process_pptx(self, filepath: str) -> Tuple[str, Dict]:
        ...
        for i, slide in enumerate(prs.slides, 1):
            ...
            for shape in slide.shapes:
                ...
                # Extract from tables on this shape (fixes loop-variable leak —
                # previously `shape` was referenced AFTER the loop, so only the
                # last shape on each slide was ever checked for tables).
