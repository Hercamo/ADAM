# ADAM Build Card — Layer 1: Code Health

**Card ID:** `qa-layer1-code-health`
**Class:** Verification Tool — Layer
**Subgroup / Plane:** QA Harness Suite — Layer 1
...
|---|---|
| Card ID | `qa-layer1-code-health` |
| Canonical Name | QA Harness Suite — Layer 1: Code Health |
| Class | Verification Tool — Layer |
...
Parse every Python module in the deployment under test and report any module that does not parse.
...
- Record each assertion as a single PASS or FAIL row through `results.check(layer=1, name, ok, detail)`.
...
Deployment root directory; canonical sub-directories for the BOSS engine, the Flight Recorder, the Cryptographic Authorization Vault, the Exception Economy, the application, the AGT integration, the arbitrator, and the digital twins.

## 6. Outputs (Flight Recorder & Side Effects)

PASS / FAIL row per Python module discovered; stdout per assertion; rows fed into the Results registry.
...
```python
def layer1_code_health(cfg: QAConfig, results: Results) -> None: ...
```

It is registered in the runner's `LAYER_FNS` dictionary under key `1`. It is invoked by the runner; it is not invoked directly by operators.
...
Hand-edit syntax error; unfinished refactor; bad merge; truncated file. Failure detail carries the parser's own error string.
...
Cheapest of the ten layers. No subprocesses, no I/O beyond the file reads.
...
| Metric | Target |
|---|---|
| Layer latency | < 5 s for the reference NetStreamX profile. |
...
- [ ] Walks every canonical sub-directory of the deployment root.
- [ ] Calls `ast.parse` on every Python module discovered.
- [ ] Records one PASS or one FAIL per module via `results.check`.
- [ ] Failure detail string is the parser's own exception string.
