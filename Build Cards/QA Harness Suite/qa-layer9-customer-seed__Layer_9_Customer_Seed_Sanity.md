# ADAM Build Card — Layer 9: Customer Seed Sanity

**Card ID:** `qa-layer9-customer-seed`
**Class:** Verification Tool — Layer
**Subgroup / Plane:** QA Harness Suite — Layer 9
...
|---|---|
| Card ID | `qa-layer9-customer-seed` |
| Canonical Name | QA Harness Suite — Layer 9: Customer Seed Sanity |
| Class | Verification Tool — Layer |
...
Spawn the deployment's seed-data generator into a temporary database and assert the synthetic-data distribution matches the configured scale.
...
- Record each assertion as a single PASS or FAIL row through `results.check(layer=9, name, ok, detail)`.
...
`netstreamx_app/seed_data.py` (or operator equivalent); configured user count, asset count, subscription-ratio band, per-asset-kind expected counts, tier-1 region floor, and minor-count band.

## 6. Outputs (Flight Recorder & Side Effects)

PASS / FAIL rows for: seed_data exits 0; user count matches; asset count matches; subscriber percentage falls in band; tier-1 region count meets floor; per-kind asset counts match; minor count falls in band.
...
```python
def layer9_customer_seed_sanity(cfg: QAConfig, results: Results) -> None: ...
```

It is registered in the runner's `LAYER_FNS` dictionary under key `9`. It is invoked by the runner; it is not invoked directly by operators.
...
Seed generator regression; scale parameters drift; per-kind distribution drift; region or age distribution drift.
...
Subprocess spawn + temporary SQLite write. Slowest layer at scale.
...
| Metric | Target |
|---|---|
| Layer latency | Reference profile: < 30 s. Production scale: minutes. |
...
- [ ] Spawns the seed generator with the configured `--users` and `--assets`.
- [ ] All §6 contracts pass against the resulting database.
