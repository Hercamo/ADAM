# ADAM Build Card — Layer 8: AI-Control Bounds

**Card ID:** `qa-layer8-ai-control`
**Class:** Verification Tool — Layer
**Subgroup / Plane:** QA Harness Suite — Layer 8
...
|---|---|
| Card ID | `qa-layer8-ai-control` |
| Canonical Name | QA Harness Suite — Layer 8: AI-Control Bounds |
| Class | Verification Tool — Layer |
...
Assert the runtime governance interface enforces the configured AI-control bounds — RL training BOSS ceiling, sacred-boundary keyword filter, and the test-profile RL-disabled default.
...
- Record each assertion as a single PASS or FAIL row through `results.check(layer=8, name, ok, detail)`.
...
AGT-Full server source; DNA JSON.

## 6. Outputs (Flight Recorder & Side Effects)

PASS / FAIL rows for: `/agt/rl/check` endpoint present; sacred-boundary keyword (or pattern) screen present; RL BOSS ceiling matches configured MODERATE-tier value; emits `rl_training_bounds_violation`; emits `rl_training_started`; DNA `runtime.rl_governance.enabled=false` in the test profile.
...
```python
def layer8_ai_control_bounds(cfg: QAConfig, results: Results) -> None: ...
```

It is registered in the runner's `LAYER_FNS` dictionary under key `8`. It is invoked by the runner; it is not invoked directly by operators.
...
Layer 8 reads the configured AI-control bounds, including BOSS-derived ceilings.
...
RL endpoint removed; ceiling drift; sacred-boundary filter weakened; test-profile default flipped.
...
Source and configuration reads.
...
- [ ] `/agt/rl/check` endpoint is present and the BOSS ceiling matches the configured tier value (default MODERATE).
- [ ] Sacred-boundary keyword (or pattern) screen is loaded into the endpoint; absence is FAIL.
- [ ] Test-profile DNA leaves `runtime.rl_governance.enabled` at the configured default (`false` in the reference profile).
