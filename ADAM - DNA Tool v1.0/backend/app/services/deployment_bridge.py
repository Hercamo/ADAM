...
class DeploymentBridge:
    ...
    async def generate_deployment(
        ...
        company_slug = session.company_name.lower().replace(" ", "-")[:30]
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        output_dir = os.path.join(self.output_base, f"adam-deployment-{company_slug}-{timestamp}")
    ...
    async def _run_generators(
        ...
        try:
            ...
            raise RuntimeError(f"DNA Deployment Tool generators not available: {e}")
