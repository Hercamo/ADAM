...
class ConfigBundleGenerator(BaseGenerator):
    ...
    def _boss_config_detailed(self) -> Dict:
        ...
        dims = boss.get("dimensions", self.get_boss_dimensions())
        priority_tiers = {
            "security_impact": "Top",
        ...
        return {
            "$schema": "https://adam.io/schemas/boss-config/v3.2",
            "_comment": "ADAM BOSS v3.2 canonical 7-dimension configuration aligned to ADAM book v1.4. "
                        "Priority Tier weights: Top=5.0 (Security), Very High=4.0 (Sovereignty, Financial), "
                        "High=3.0 (Regulatory, Reputational, Rights), Medium=2.0 (Doctrinal). Denominator \u03a3 = 24.0.",
            "dimensions": dims,
            "priority_tiers": priority_tiers,
            "composite_formula": {
                "type": "weighted_average",
                "weight_sum": 24.0,
                "formula": "C = (S_sec*5.0 + S_sov*4.0 + S_fin*4.0 + S_reg*3.0 + S_rep*3.0 + S_rights*3.0 + S_doc*2.0) / 24.0",
                "critical_override": "if max(S_d) > 75: C = max(C, max(S_d) - 10)",
                "non_idempotent_penalty": 15,
                "non_idempotent_penalty_type": "additive_flat",
                "cap": 100,
            },
...
# Canonical BOSS v3.2 Priority Tier weights (denominator Sigma = 24.0)
...
composite_score := score {{
  ...
  # Rego has no inline ternary — select `base` via two guarded rules below.
  max_dim := max([input.dimensions[d] | some d in object.keys(weights)])
  override := max_dim - 10

  base := _base_with_override(raw, override, max_dim)

  # Non-Idempotent Penalty: +15 flat additive (NOT a multiplier).
  penalty := _penalty(input.is_non_idempotent)

  score := min([base + penalty, 100])
}}

# Pick raw score unless a single dimension tripped the >75 threshold,
# in which case apply the Critical Dimension Override.
_base_with_override(raw, override, max_dim) := out {{
  max_dim > 75
  out := max([raw, override])
}}
_base_with_override(raw, _, max_dim) := raw {{
  max_dim <= 75
}}

# Non-idempotent +15 additive penalty.
_penalty(true) := 15
_penalty(false) := 0

# Routing tier (SOAP/MODERATE/ELEVATED/HIGH/OHSHAT per canonical BOSS v3.2).
# Rego rule style: each guarded rule is its own body; OPA picks the matching one.
default routing_tier := "soap"
routing_tier := "ohshat" {{ composite_score > 75 }}          # 76-100
routing_tier := "high" {{ composite_score > 50; composite_score <= 75 }}   # 51-75
routing_tier := "elevated" {{ composite_score > 30; composite_score <= 50 }} # 31-50
routing_tier := "moderate" {{ composite_score > 10; composite_score <= 30 }} # 11-30
...
# Director authorization check (5-Director Constitution; CPO/CTO optional).
# A request is director-authorized if (a) the source IS a seated director, OR
# (b) the request arrives with a non-empty delegation chain back to a director.
director_authorized {{
