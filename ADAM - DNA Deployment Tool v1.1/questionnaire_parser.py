...
class DNAQuestionnaireParser:
    ...
    def _parse_boss_dimensions(self, text: str) -> Dict[str, float]:
        ...
        # Fallback defaults if parsing fails: ADAM BOSS v3.2 canonical 7-dimension set.
        # Priority Tiers: Top=5.0 (Security), Very High=4.0 (Sovereignty, Financial),
        # High=3.0 (Regulatory, Reputational, Rights), Medium=2.0 (Doctrinal). Denom = 24.0.
        if not dimensions:
            dimensions = {
                "security_impact": 5.0,
    ...
    def _parse_boss_thresholds(self, text: str) -> Dict[str, Dict[str, int]]:
        """Parse BOSS routing thresholds (ADAM book v1.4 canonical 5 tiers)."""
        thresholds = {
            ...
            "ohshat": {"min": 76, "max": 100, "label": "OHSHAT - Operational Hell, Send Humans Act Today!"},
        }
        return thresholds
