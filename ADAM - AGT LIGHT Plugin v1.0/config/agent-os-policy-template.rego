...
# policy data is generated from the DNA Questionnaire via the DNA Deployment Tool.
# ══════════════════════════════════════════════════════════════════════════════
...
_deny_reason := msg if {
    ...
    msg := sprintf("BOSS score %.1f exceeds HIGH threshold (75). OHSHAT protocol active. Director approval required.", [input.boss_scores.composite])
}
...
_deny_reason := msg if {
    ...
    msg := sprintf("BOSS score %.1f exceeds SOAP threshold (10). Autonomous execution denied. Escalation required.", [input.boss_scores.composite])
}
