...
Version: 1.1.0  (BOSS v3.2 canonical weights, +15 additive non-idempotent
                 penalty, Critical Dimension Override threshold 75/-10,
                 canonical 5 Governor Agents, 5-Director Constitution)
Python: 3.10+
"""
...
class BOSSScore:
    ...
    # Canonical BOSS v3.2 Priority Tier weights (ADAM book v1.4)
    #   Security Impact      5.0  (Top)
    #   Sovereignty Action   4.0  (Very High)
    #   Financial Exposure   4.0  (Very High)
    #   Regulatory Impact    3.0  (High)
    #   Reputational Risk    3.0  (High)
    #   Rights Certainty     3.0  (High)
    #   Doctrinal Alignment  2.0  (Medium)
    # Σ weights = 24.0 (denominator)
    _V32_WEIGHTS = {
        "security_impact":     5.0,
    ...
    _V32_DENOMINATOR = 24.0

    @property
    def composite_score(self) -> float:
        """Composite BOSS score via canonical v3.2 weighted average (Σ = 24.0).

        Formula (ADAM book v1.4 / BOSS Score Formulas v3.2):
          C = (S_sec×5 + S_sov×4 + S_fin×4 + S_reg×3 + S_rep×3 + S_rights×3 + S_doc×2) / 24

        Critical Dimension Override and +15 additive non-idempotent penalty
        are applied downstream in `composite_score_adjusted` when the caller
        supplies the appropriate context.
        """
        weighted_sum = (
            self.security_impact     * 5.0
            + self.sovereignty_action * 4.0
            + self.financial_exposure * 4.0
            + self.regulatory_impact  * 3.0
            + self.reputational_risk  * 3.0
            + self.rights_certainty   * 3.0
            + self.doctrinal_alignment * 2.0
        )
        return weighted_sum / 24.0

    def composite_score_adjusted(
        self,
        non_idempotent: bool = False,
        critical_dim_threshold: float = 75.0,
        critical_dim_floor_offset: float = -10.0,
    ) -> float:
        """Composite BOSS score with canonical v3.2 adjustments applied.

        - Critical Dimension Override: if any single dimension exceeds
          `critical_dim_threshold` (default 75), the composite is raised to
          at least max(dim) + critical_dim_floor_offset (default -10).
        - Non-Idempotent Penalty: +15 flat additive when `non_idempotent=True`.

        Result is clamped to [0, 100].
        """
        base = self.composite_score
        max_dim = max(
            getattr(self, dim.value) for dim in BOSSDimension
        )
        if max_dim > critical_dim_threshold:
            base = max(base, max_dim + critical_dim_floor_offset)
        if non_idempotent:
            base = base + 15.0
